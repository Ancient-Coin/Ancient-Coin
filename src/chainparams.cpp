...
public:
    CMainParams() {
        ...
        consensus.ANCHeight = 491407; // Around 10/25/2017 12:00 UTC
        consensus.ANCPremineWindow = 8000;
        consensus.ANCPremineEnforceWhitelist = true;
...
public:
    CTestNetParams() {
        ...
        consensus.ANCHeight = 1210320;
        consensus.ANCPremineWindow = 50;
        consensus.ANCPremineEnforceWhitelist = false;
...
public:
    CRegTestParams() {
        ...
        consensus.ANCHeight = 3000;
        consensus.ANCPremineWindow = 10;
        consensus.ANCPremineEnforceWhitelist = false;
...
bool CChainParams::IsPremineAddressScript(const CScript& scriptPubKey, uint32_t height) const {
    ...
    assert((uint32_t)consensus.ANCHeight <= height &&
           height < (uint32_t)(consensus.ANCHeight + consensus.ANCPremineWindow));
    if (!consensus.ANCPremineEnforceWhitelist) {
        return true;
    }
    int block = height - consensus.ANCHeight;
    int num_unlocked = consensus.ANCPremineWindow * 40 / 100;  // 40% unlocked.
    int num_locked = consensus.ANCPremineWindow - num_unlocked;  // 60% time-locked.
    ...
    if (block < num_unlocked) {
        ...
        int lock_time = consensus.ANCHeight + stage_lock_time * (1 + stage);
