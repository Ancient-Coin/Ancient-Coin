...
const std::string strMessageMagic = "Ancient Coin Signed Message:\n";
...
bool static IsANCHardForkEnabled(int nHeight, const Consensus::Params& params) {
    return nHeight >= params.ANCHeight;
}

bool IsANCHardForkEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params) {
    if (pindexPrev == nullptr) {
        return false;
    }

    return IsANCHardForkEnabled(pindexPrev->nHeight, params);
}

bool IsANCHardForkEnabledForCurrentBlock(const Consensus::Params& params) {
    AssertLockHeld(cs_main);
    return IsANCHardForkEnabled(chainActive.Tip(), params);
}
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    bool postfork = block.nHeight >= (uint32_t)consensusParams.ANCHeight;
...
bool IsInitialBlockDownload()
{
    ...
    if (chainActive.Tip()->nChainWork < UintToArith256(chainParams.GetConsensus().nMinimumChainWork))
        return true;
    if (fSkipHardforkIBD && chainActive.Tip()->nHeight + 1 >= (int)chainParams.GetConsensus().ANCHeight)
        return false;
    int64_t target_time = fANCBootstrapping ? (int64_t)chainParams.GetConsensus().BitcoinPostforkTime : GetTime();
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    if (IsANCHardForkEnabled(pindex->pprev, consensusparams)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.nHeight < (uint32_t)consensusParams.ANCHeight) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check block height for blocks after ANC fork.
    if (nHeight >= consensusParams.ANCHeight && block.nHeight != (uint32_t)nHeight)
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.ANCHeight &&
        nHeight < consensusParams.ANCHeight + consensusParams.ANCPremineWindow)
    {
